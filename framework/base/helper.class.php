...
class baseHelper
{
    ...
    static public function isValidateDate($date, $format = 'Y-m-d H:i:s')
    {
        $d = DateTime::createFromFormat($format, $date);
        return $d && $d->format($format) == $date;
    }

    static public function showYMD($dateStr, $format = 'Y-m-d')
    {
        $d = DateTime::createFromFormat( 'Y-m-d H:i:s', $dateStr);
        if($d)
        {
        ...
        {
            error_log("helper::showYMD error date string:{$dateStr}");
            return $dateStr;
        }
    }


    static public function getWorkingDays($endDate, $startDate, $holidays = array()){
    ...
    static public function dayafter($from, $days)
    {
        $now = date($from);
        $d = "$days days";
        $date = date_create($now);
