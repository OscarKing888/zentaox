...
<div id='unlinkBugList'>
  <form method='post' id='unlinkedBugsForm' target='hiddenwin' action='<?php echo $this->createLink('buildex', 'linkBug', "buildID={$build->id}&browseType=$browseType&param=$param");?>'>
    <table class='table table-condensed table-hover table-striped tablesorter table-fixed table-selectable'> 
      ...
      <?php
          if(strpos(",{$build->bugs},", ",$bug->id,") !== false) continue;
       ?>
      <?php
          if($build->product != $bug->product) continue;
      ?>
      <tr>
        <td class='cell-id'>
      ...
      <tr>
        <td colspan='6' class='text-left'>
          <?php if(count($allBugs))
          {
              ...
              echo html::selectButton() . html::submitButton($lang->buildex->linkBug);
