...
?>
<?php
class buildexModel extends model
{
    /**
     * Get build info.
     *
    ...
    public function getByID($buildID, $setImgSize = false)
    {
        ...
        $build->files = $this->file->getByObject('buildex', $buildID);
    ...
    {
        return $this->dao->select('*')->from(TABLE_BUILDEX)->where('id')->in($idList)->fetchAll('id');
    }
    ...
    public function getProjectBuildPairs($projectID, $productID, $branch = 0, $params = '')
    {
        ...
        $projectBuilds = $this->dao->select('t1.id, t1.name, t1.project, t2.status as projectStatus, t3.id as releaseID, t3.status as releaseStatus')->from(TABLE_BUILDEX)->alias('t1')
            ->leftJoin(TABLE_PROJECT)->alias('t2')->on('t1.project = t2.id')
    ...
    public function getProductBuildPairs($products, $branch = 0, $params = 'noterminate, nodone', $replace = true)
    {
        ...
        $productBuilds = $this->dao->select('t1.id, t1.name, t1.project, t2.status as projectStatus, t3.id as releaseID, t3.status as releaseStatus')->from(TABLE_BUILDEX)->alias('t1')
            ->leftJoin(TABLE_PROJECT)->alias('t2')->on('t1.project = t2.id')
            ->leftJoin(TABLE_RELEASE)->alias('t3')->on('t1.id = t3.build')
            //->leftJoin(TABLE_BRANCH)->alias('t4')->on('t1.branch = t4.id')
    ...
    {
        return $this->dao->select('id, name')->from(TABLE_BUILDEX)
    ...
    public function create($projectID)
    {
        ...
        $build = fixer::input('post')
            ...
            ->stripTags($this->config->buildex->editor->create['id'], $this->config->allowedTags)
            ->remove('resolvedBy,allchecker,files,labels,uid')
            ->get();

        if($this->config->global->flow == 'onlyTest') $build->project = 0;

        //$build = $this->loadModel('file')->processImgURL($build, $this->config->buildex->editor->create['id'], $this->post->uid);
        //var_dump($build);

        /*
         foreach ($build as $k => $v ) {
            error_log("buildex k:{$k} v:{$v}");
        }
        //*/

        $this->dao->insert(TABLE_BUILDEX)->data($build)
            ->autoCheck()
            ->batchCheck($this->config->buildex->create->requiredFields, 'notempty')
            ->check('name', 'unique', "product = {$build->product} AND deleted = '0'")
            ->exec();
        ...
        if(!dao::isError())
        {
            ...
            //$this->file->saveUpload('buildex', $buildID);
            //$this->loadModel('score')->create('buildex', 'create', $buildID);
            //return $buildID;
    ...
    {
        $oldBuild = $this->dao->select('*')->from(TABLE_BUILDEX)->where('id')->eq((int)$buildID)->fetch();

        $build    = fixer::input('post')->stripTags($this->config->buildex->editor->edit['id'], $this->config->allowedTags)
            ->remove('allchecker,resolvedBy,files,labels,uid,scope')
            ->get();
        ...
        //$build = $this->loadModel('file')->processImgURL($build, $this->config->buildex->editor->edit['id'], $this->post->uid);
        $this->dao->update(TABLE_BUILDEX)->data($build)
            ->autoCheck()
            ->batchCheck($this->config->buildex->edit->requiredFields, 'notempty')
    ...
    public function linkStory($buildID)
    {
        ...
        $this->dao->update(TABLE_BUILDEX)->set('stories')->eq($build->stories)->where('id')->eq((int)$buildID)->exec();
    ...
    public function linkBug($buildID)
    {
        ...
        $this->dao->update(TABLE_BUILDEX)->set('bugs')->eq($build->bugs)->where('id')->eq((int)$buildID)->exec();
    ...
    public function updatePathInfo($buildID, $fieldName, $newVal)
    {
        $build = $this->getByID($buildID);

        $newBuild = new stdClass();
        $newBuild->$fieldName = $newVal;

        $confirmField = $fieldName . 'Confirm';
        $newBuild->$confirmField = 1;

        //error_log("updatePathInfo fieldName:{$fieldName} confirm:{$confirmField}");

        $this->dao->update(TABLE_BUILDEX)->data($newBuild)->where('id')->eq((int)$buildID)->exec();

        $changes[] = array('field' => $fieldName, 'old' => $build->$fieldName, 'new' => $newVal);
