...
function drawProjectBlueprint(tasks)
{
    ...
    //*
    var idx = 0;
    drawTask(ctx, "1 CRASH自动收集分析",  "2018-4-30", 24, 'done', idx++);
    drawTask(ctx, "2 CRASH自动收集分析",  "2018-4-30", 32, 'doing', idx++);
    drawTask(ctx, "3 CRASH自动收集分析",  "2018-5-1", 40, 'wait', idx++);
    drawTask(ctx, "4 CRASH自动收集分析",  "2018-5-5", 48, 'wait', idx++);
    //*/
    ...
    if(tasks != null)
    {
        ...
        {
            drawTask(ctx, tasks[i].name,  tasks[i].estStarted, tasks[i].estimate, tasks[i].status, i);
        }
    }
    //alert("canvas:" + canvas.id + " docW:" + document.width);
...
function drawTask(ctx, name, startDate, hours, status, idx)
{
    ...
    ctx.fillStyle = C_TaskNameColor;
    ctx.font = C_TaskNameFont;
    ctx.fillText(name, xyend[0] + 8, xyend[1] + C_FontHeight);
}
