...
class projectModel extends model
{
    ...
    public function checkPriv($project)
    {
        ...
        static $teams;
        if(empty($teams)) $teams = $this->dao->select('root, account')->from(TABLE_TEAM)->fetchGroup('root', 'account');
    ...
    public function select($projects, $projectID, $currentModule, $currentMethod, $extra = '')
    {
        ...
        $output = "<a id='currentItem' href=\"javascript:showSearchMenu('project', '$projectID', '$currentModule', '$currentMethod', '$extra')\"><div class='red'>{$currentProject->name} <span class='icon-caret-down'></span></div></a><div id='dropMenu'><i class='icon icon-spin icon-spinner'></i></div>";
        if($isMobile) $output  = "<a id='currentItem' href=\"javascript:showSearchMenu('project', '$projectID', '$currentModule', '$currentMethod', '$extra')\"><div class='red'>{$currentProject->name} <span class='icon-caret-down'></span></div></a><div id='currentItemDropMenu' class='hidden affix enter-from-bottom layer'></div>";
        return $output;
    }
    ...
    public function create($copyProjectID = '')
    {
        ...
        if(!dao::isError())
        {
            ...
            if($copyProjectID != '') 
            {
                $members = $this->dao->select('*')->from(TABLE_TEAM)->where('root')->eq($copyProjectID)->fetchAll();
    ...
    public function getById($projectID, $setImgSize = false)
    {
        ...
        $project->totalHours    = $this->dao->select('sum(days * hours) AS totalHours')->from(TABLE_TEAM)->where('root')->eq($project->id)->fetch('totalHours');
    ...
    public function importTask($projectID)
    {
        ...
        foreach($assignedToes as $account => $preProjectID)
        {
            ...
            {
                $role = $this->dao->select('*')->from(TABLE_TEAM)->where('root')->eq($preProjectID)->andWhere('account')->eq($account)->fetch();
    ...
    {
        $this->dao->delete()->from(TABLE_TEAM)->where('root')->eq((int)$projectID)->andWhere('account')->eq($account)->exec();
    }
    ...
    public function getLimitedProject()
    {
        ...
        /* Get all teams of all projects and group by projects, save it as static. */
        $projects = $this->dao->select('root, limited')->from(TABLE_TEAM)->where('account')->eq($this->app->user->account)->andWhere('limited')->eq('yes')->orderBy('root asc')->fetchPairs('root', 'project');
    ...
    public function buildBugSearchForm($products, $queryID, $actionURL)
    {
        ...
        foreach($products as $product)
        {
            ...
            $productBuilds  = $this->loadModel('buildex')->getProductBuildPairs($product->id, 0, $params = 'noempty|notrunk');
    ...
    {
        //$milestone = $this->dao->findById((int)$milestoneId)->from(TABLE_PRODUCTMILESTONE)->fetch();

        $milestone = $this->dao->select()->from(TABLE_PRODUCTMILESTONE)
...
//        }
    ...
    public function updateMilestone($projectId, $milestoneId)
    {
        $oldMilestone = $this->getMilestoneById($milestoneId);

        $milestone = new stdClass();
        $milestone->name =  $this->post->name;
        $milestone->deadlineRequirement =  $this->post->deadlineRequirement;
        $milestone->deadline =  $this->post->deadline;
        $milestone->deadlineQA =  $this->post->deadlineQA;
        $milestone->opOnlineDate =  $this->post->opOnlineDate;

        $this->dao->update(TABLE_PRODUCTMILESTONE)->data($milestone)
            ->autoCheck()
            ->where('id')->eq((int)$milestoneId)
            ->exec();

        if(!dao::isError())
        {
            return common::createChanges($oldMilestone, $milestone);
        }
    }
}
